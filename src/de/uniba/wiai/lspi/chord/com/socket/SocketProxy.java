...
public final class SocketProxy extends Proxy implements Runnable {
	 ...
	 * @throws CommunicationException
	 */
        //modified(inspired from removeReplicas)
        public void dqdhtProcess(Serializable s , ID askingNodeId, ID limitNodeId)
                throws CommunicationException{
            this.makeSocketAvailable();
            logger.debug("Trying to execute dqdht process ");
            Request request = this.createRequest(MethodConstants.DQDHT_PROCESS,
                    new Serializable[]{s, askingNodeId, limitNodeId});
            try {
			logger.debug("Trying to send request " + request);
			this.send(request);
		} catch (CommunicationException ce) {
